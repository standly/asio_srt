...
public:
    ...
    awaitable<void> connect() {
        ...
        try {
            while (active_) {
                auto [ec, event] = co_await queue_->async_read_msg(use_awaitable);
                
                if (ec) {
                    std::cout << "[User:" << user_id_ << "] Disconnected: " << ec.message() << std::endl;
                    break;
                }
    ...
    std::string name)
{
    ...
    try {
        while (true) {
            // Read stock updates
            auto [ec, update] = co_await queue->async_read_msg(use_awaitable);
